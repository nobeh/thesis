...
\TODO{Put a titled paraph here? e.g. ``Concurrency Approaches''}
...
\TODO{and this property makes compositional concurrency of coroutines straightforward}.
...
Future values can be used to hold the eventual return value of a method at the call site. \TODO{Do we need a citation for futures?}
...
\TODO{Discuss the existence of this table. Where's concurrency? Remove ``Coroutine'' row. Add a column ``Concurrency''. Fix ``Execution'' column.}
...
\\ \midrule
\emph{Concurrency} & Mutli-threading & Multi-threading + RTC
\\ \bottomrule
\end{tabular}
...
In addition, the Java language specification (JLS)~\cite{gosling2000java,jls8} clearly reveals that the Java language is not yet ready to support algebraic data types.
...
We aim to rely only on the mainstream JVM platform released by the Java team at Oracle; in addition, we do not intend to use byte-code engineering.
...
\TODO{Where does the following paragraph have to move?}
...
In distributed systems, the number of messsages delivered among objects in the 
