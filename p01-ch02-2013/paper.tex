...
\end{align}
\caption[A kernel version of the real-time programming language]{
...
\label{ch02:fig:grammar}
\end{figure}

\paragraph*{How to send a message with a deadline?} 
...
\paragraph*{How to process the response of a message with a deadline?}
...
% \vspace{-20pt}
\begin{lstlisting}[float=h, label=ch02:lst:try:catch, caption=Using try-catch for processing future values]
try {
	x = f.get(e)
	S_1
} catch(Exception x) {
...
% Since, we provide a reference implementation in Java (c.f. Section \ref{ch02:sec:impl}), we take advantage of exception ``stack traces''; 
...
\subsection{Case Study: Fredhopper Distributed Data Processing} \label{ch02:sec:frh}
...
\end{center}
% \vspace{-20pt}
\caption[Life cycle for remote data processing]{Fredhopper's Controller life cycle for remote data processing}
\label{ch02:fig:frh-dm}
% \vspace{-10pt}
% \end{wrapfigure}
\end{figure}
...
\section{Operational Semantics} \label{ch02:sec:sos}
...
\paragraph*{Initialization of \jtt{timeout} in \gett  \space operation} 
...
\paragraph*{The \textnormal{\textbf{\jtt{try-catch}}} block} semantics is presented by: 
...
\paragraph*{Abnormal termination} of the active process is generated by an uncaught exception:
$$
\frac
{(S,\sigma,\phi)\uparrow v}
...
% Additionally, as we discssued in Equation \ref{ch02:eq:orderedq}, each concurrent object owns an ordered queue for the processes.
...
\section{Implementation} \label{ch02:sec:impl}
...
%\paragraph*{Implementation of $(S, \tau)$ and $(S, \sigma)$.} 
...
% Controller operates in a high-demand distributed setting as in Figure \ref{ch02:fig:ctl:deploy}.
...
% \vspace{-25pt}
\begin{lstlisting}[label=ch02:lst:frh:polling, caption=With polling]
...
\begin{minipage}[t]{0.48\textwidth}
% \vspace{-25pt}
\begin{lstlisting}[label=ch02:lst:frh:deadlines, caption=With deadlines]
class DataProcessor begin
	op process(d: Data) == 
		var p := allocDataProcessor(d)
...
\begin{minipage}[t]{0.48\textwidth}
% \vspace{-30pt}
\begin{lstlisting}[label=ch02:lst:frh:dm, caption=Data Processor class]
class DataProcessor begin
...
\begin{minipage}[t]{0.48\textwidth}
% \vspace{-30pt}
\begin{lstlisting}[label=ch02:lst:frh:sched, caption=Custom scheduler]
...
To use the custom scheduler, the only requirement is that the class \jtt{DataProcessor} defines a specific class variable called \jtt{scheduler} in Listing \ref{ch02:lst:frh:dm}.
...
\section{Conclusion and future work} \label{ch02:sec:conclusion}
